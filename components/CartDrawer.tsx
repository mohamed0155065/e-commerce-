...
export const CartDrawer = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div className={`fixed top-0 right-0 h-screen w-full max-w-xl bg-white shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? "translate-x-0" : "translate-x-full"}`} role="dialog" aria-modal="true" aria-label="Shopping cart">
                ...
                <div className="flex flex-col h-full">
                        ...
                        {items.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center">
                                <div className="w-32 h-32 bg-slate-50 rounded-full flex items-center justify-center mb-6">
                                    <ShoppingBag size={50} className="text-slate-200" aria-hidden="true" />
                                </div>
                                ...
                                <button onClick={onClose} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-600 transition" aria-label="Browse shop">
                                    Browse Shop
                                </button>
                            </div>
                        ) : (
                                ...
                                {items.map((item) => (
                                    <div key={item.id} className="flex gap-6 items-center group">
                                        ...
                                        <div className="w-32 h-32 bg-slate-50 rounded-[2rem] overflow-hidden border border-slate-100 flex-shrink-0 p-4 transition-transform group-hover:scale-105">
                                            <img src={item.Image} className="w-full h-full object-contain" alt={`Image of ${item.Name}`} />
                                        </div>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center justify-between mt-4">
                                                {/* Quantity Controls */}
                                                <div className="flex items-center bg-slate-100 rounded-2xl p-1 gap-1" role="group" aria-label={`Quantity controls for ${item.Name}`}>
                                                    <button
                                                    ...
                                                    <span className="text-sm font-black text-slate-900 w-10 text-center" aria-live="polite">{item.quantity}</span>
                                                    <button
                    ...
                    {items.length > 0 && (
                        <div className="px-8 py-8 border-t border-slate-100 bg-white shrink-0 shadow-[0_-20px_40px_rgba(0,0,0,0.02)]">
                            ...
                            >
                                Proceed to Checkout
                                <ArrowRight size={22} className="group-hover:translate-x-2 transition-transform duration-300" aria-hidden="true" />
                            </Link>
                        </div>
                    )}
                </div>
